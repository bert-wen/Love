function showTime() { 
	...
	var startDate = new Date('2020-12-14 00:00:00');
	var date3 = currentDate.getTime()/1000-startDate.getTime()/1000;
	//计算天数
	var days=Math.floor(date3/(24*3600));
	var daysMinutes=days*24*3600;
	//计算天数后剩余的秒数
	var tempHours=date3-daysMinutes;
	var hours=Math.floor(tempHours/(3600));
	//计算小时数后剩余的秒数
	var tempMinutes=tempHours-hours*3600;
	var minutes=Math.floor(tempMinutes/(60));
	//计算分钟数后剩余的毫秒数
	var tempSeconds=tempMinutes-minutes*60;
	var seconds=Math.round(tempSeconds);
	if (minutes < 10) 
	minutes = "0" + minutes; 
	if (seconds < 10) 
	seconds = "0" + seconds; 
	var currentTimeString = "Dear XiaoYan:<br>"
		...
		+	"<c style=\"color: #CCCCCC; text-shadow:2px 3px 3px #222; font-family:Microsoft YaHei; font-size:20px\" >----WenHui 2020-12-14</c>";
